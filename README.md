...
Бот умеет:
- анализировать отдельную акцию по тикеру;
- выдавать базовые сигналы (SMA20/50, RSI14);
- рисовать график цены + индикаторы;
- делать AI-обзор новостей по тикеру (если есть `OPENAI_API_KEY`);
- анализировать существующий портфель;
...
- Проверит и остановит все старые экземпляры бота
- Запустит новый экземпляр
- Сохранит PID процесса
- Гарантирует что запущен только один экземпляр
...
- Предупреждение если запущено несколько экземпляров

**Остановка бота:**
```bash
./stop_bot.sh
```
Останавливает все работающие экземпляры бота.
...
### 📝 Ручной запуск (не рекомендуется)
```bash
python bot.py
```
⚠️ При ручном запуске можно случайно запустить несколько экземпляров, что приведет к ошибкам.

## Формат ввода в боте
### Анализ акции
Просто тикер, например:
- `AAPL`
- `MSFT`
...
- Рыночные данные тянутся через `yfinance`.
- Если `OPENAI_API_KEY` не задан, бот делает базовый обзор новостей без LLM.
