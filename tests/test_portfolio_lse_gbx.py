...
class _MockProviderNoLseNetwork:
    async def get_price_history(self, ticker, period="7d", interval="1d", min_rows=2):
        # LSE UCITS tickers should use immediate synthetic fallback from avg_price.
        if ticker in {"VWRA.L", "SGLN.L", "AGGU.L", "SSLN.L"}:
            raise AssertionError(f"unexpected network fetch for {ticker}")
        if ticker == "SPY":
            return pd.DataFrame({"Close": [500.0 + i for i in range(90)]}), None
        return pd.DataFrame({"Close": [100.0 + i for i in range(90)]}), None
...
def test_lse_gbx_quotes_are_normalized_in_report():
    ...
    # GBX->GBP normalization must always apply for SGLN.
    assert "SGLN: qty 25.0, price 72." in text
...
def test_lse_positions_use_fast_local_fallback_without_network_calls():
    positions = [
        Position(ticker="VWRA", quantity=80.0, avg_price=172.25),
        Position(ticker="SGLN", quantity=25.0, avg_price=7230.0),
        Position(ticker="AGGU", quantity=25.0, avg_price=5.816),
    ...
    text = asyncio.run(analyze_portfolio(positions, _MockProviderNoLseNetwork()))

    assert "- VWRA:" in text
    assert "- SGLN:" in text
    assert "- AGGU:" in text
    assert "- SSLN:" in text
    assert "Риск-метрики (1Y):" in text
